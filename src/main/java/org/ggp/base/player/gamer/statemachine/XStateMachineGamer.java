...
import org.ggp.base.util.statemachine.XORShiftRandom;
...
public abstract class XStateMachineGamer extends Gamer
{
   ...
   /* protected final void switchStateMachine(StateMachine newStateMachine) {
	...
	protected void saveMachine() {
		...
		String filename = "statemachine" + numRules + ".goat";
		try {
            ...
            System.out.println("Saved state machine in " + filename);
	...
	protected XStateMachine loadStateMachine() {
		int numRules = getMatch().getGame().getRules().size();
		String filename = "statemachine" + numRules + ".goat";
		// Deserialization
		XStateMachine savedMachine = null;
        try
        {
            ...
            savedMachine = (XStateMachine) in.readObject();
            if (savedMachine != null) {
            	savedMachine.rand = new XORShiftRandom();
            }

            in.close();
            file.close();

            System.out.println("Loaded state machine from " + filename);
        }
